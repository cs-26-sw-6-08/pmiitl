...
fn should_fail() {
    ...
    // Always sum sum number = fail
    assert!(always_expr(function_expr(FunctionType::Sum, function_expr(FunctionType::Sum, number_expr(), None), None)).monitorability_check().is_err());
    ...
    // Always binary always or avgtime count = fail
    assert!(always_expr(binary_expr(always_expr(number_expr()), function_expr(FunctionType::Avgtime, function_expr(FunctionType::Count, number_expr(), None), Some(number_expr())), BinaryOperators::Or)).monitorability_check().is_err());
}
